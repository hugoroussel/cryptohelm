                ...
                <tbody className="divide-y divide-gray-200">
                  ...
                  {pageProps.contracts.map((contract) => (
                    <div className="grid grid-cols-7 items-center hover:bg-gray-400" key={contract.address}>
                      ...
                      <div>
                        <ArrowTopRightOnSquareIcon className='h-5 w-5 ml-2 hover:text-blue-500' onClick={(e)=>{e.preventDefault(),window.open(returnBlockchainExplorerLinkWithChainId(contract?.verifiedon[0])+'/address/'+contract.address, '_blank');}}/>
                      </div>
                    </div>
                  ))}
                </tbody>
