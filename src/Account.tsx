...
function Account(pageProps :AccountPageProps) {
  ...
  useEffect(() => {
    ...
    async function readChromeExtensionsVersions(){
      const res = await axios.get(`${process.env.REACT_APP_SERVER_URL}/extensions`);
    ...
    <body className='w-[380px]'>
      ...
      <div>
        ...
        <dl className="mt-5 grid grid-cols-2 m-3 gap-1">
          {stats.map((item) => (
            <div key={item.name} className="overflow-hidden rounded-lg bg-white px-4 py-5 shadow-inner sm:p-6">
